...
class LocalAligner {
  constructor(array1, array2, {gap, mismatch, match}) {
    this._q = array1;
    this._p = array2;
    this._gapPen = gap;
    ...
    this._matrixA = zeros2D(array1.length + 1, array2.length + 1);
    this._matrixB = zeros2D(array1.length + 1, array2.length + 1);
